...
§
Coding workflow: load `pedro-coding-agent-workflow`. Lua PMs software builds: strong prompts/plan mode, GitHub issues/PRs, parallel Claude/Codex, early context compaction. Long agent runs use tmux/Hermes background with notify_on_complete, not polling. Interactive Claude sessions enable `/remote-control`. Codex: gpt-5.5+xhigh. Claude: effort max+bypass.
§
