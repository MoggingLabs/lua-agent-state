...
§
Lead-gen AdOps OS PRD decisions: browser SaaS MVP; local runner later; BYOK cloud vault first/local-only later; Meta launch package export first; monitoring pulls Meta+GHL with private-beta kill/scale recommendations; native workflow core, ClickUp export later; shared creative base + media extensions; client profiles combine required structured fields with freeform docs/extracted fields; paid design partner motion; white-label architecture-ready, not MVP UI.
§
